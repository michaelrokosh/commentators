class User < ActiveRecord::Base
  ...
  def voted_for?(voteable)
    votes.where(voteable_id: voteable.id, voteable_type: voteable.class.name).first.present?
  end

  def positive_vote?(voteable)
    votes.where(voteable_id: voteable.id, voteable_type: voteable.class.name).first.rate == 1
  end
